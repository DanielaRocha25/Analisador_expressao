...
def main():
    lista_exp = {'chave':'exp'} #Inializei o dicionario
    for i in range(65,90): #feito com base na tabela ascii (de A-Z)
        while True:
            expressao_param = input(f'Informe uma atribuição ({chr(i)}) : ') #Recebe uma atribuição ex: 2+3*2
            if expressao_param=="": #Vai parar quando não informar nenhum valor e der enter
                break
            else:
                lexer_param = Analise_lexica(expressao_param) #Chamo a analis. lexica pra avaliar a expressao
                parse_param = Analise_sintatica(lexer_param) #Chamo a analis. sintatica .
                if parse_param.token_atual.type != ERRO:
                    lista_exp.update({chr(i):expressao_param}) #insiro o valor no dicionario
                    break
                else:
                    print("Expressão inválida, informe novamente!")
        if expressao_param == "":
            break

    expressao = input('Informe uma expressão aritmética para verificar: ').upper()
    orig_expressao = expressao
    for chave in lista_exp.keys():
        expressao =expressao.replace(chave, str(lista_exp[chave]))
    print(f"Expressão informada: {orig_expressao}")
    print(f"Expressão formatada: {expressao}")
